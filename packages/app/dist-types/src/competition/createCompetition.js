import { DEFAULT_OPPONENT_GRID_SLOTS, STARTING_GRID_SLOT_COUNT, getStartingGridSlot, } from '@indygp/core';
...
function signedTrackDelta(fromS, toS, length) {
    let delta = wrapS(toS - fromS, length);
    if (delta > length * 0.5)
        delta -= length;
    return delta;
}
function sampleCentreline(CL, s) {
...
function curvatureSpeedEnvelope(state, CL) {
    const lookAhead = [0, 10, 20, 32, 46, 62, 78];
    const lateralAccel = 17.5 *
    ...
    for (const distance of lookAhead) {
        ...
        const curveSpeed = clamp(Math.sqrt(lateralAccel / curvature), minCurveSpeed, maxCurveSpeed);
        const currentSpeedAllowance = Math.sqrt(curveSpeed * curveSpeed + 2 * braking * distance);
        allowed = Math.min(allowed, currentSpeedAllowance);
    }
    return allowed;
}
function turnSignAt(CL, turnS) {
...
function nearestTurnContext(state, CL, TURNS) {
    let best = null;
    for (const turn of TURNS) {
        const distance = signedTrackDelta(state.s, turn.s, CL.length);
        if (distance < -48 || distance > 118)
            continue;
...
function racingLineOffset(context) {
    if (!context)
        return 0;
...
function markerSpeedEnvelope(state, CL, TURNS, cruise, rhythm) {
    ...
    for (const turn of TURNS) {
        ...
        const cornerSpeed = clamp(cornerBase *
            state.driver.cornerSkill *
            state.driver.pace *
            clamp(rhythm, 0.97, 1.02), cornerFloor, cornerCeiling);
        if (signed >= 0) {
            const entryBlend = clamp01((92 - signed) / 72);
            target = Math.min(target, cruise + (cornerSpeed - cruise) * entryBlend);
            continue;
        }
        const exitProgress = clamp01((-signed - 8) / 44);
        const exitTarget = cornerSpeed + (cruise - cornerSpeed) * exitProgress;
        target = Math.min(target, exitTarget);
    }
    return target;
}
function targetSpeedFor(state, CL, TURNS) {
    const rhythm = driverRhythm(state);
    const cruise = 72 * state.driver.pace * rhythm;
    const markerCap = markerSpeedEnvelope(state, CL, TURNS, cruise, rhythm);
    const curvatureCap = curvatureSpeedEnvelope(state, CL);
    return clamp(Math.min(cruise, markerCap, curvatureCap), TIGHT_CORNER_MIN, 76);
}
function interactionFor(state, states, CL) {
    /*
    ...
    for (const other of states) {
        ...
        if (delta <= 0 || delta >= nearestDistance)
            continue;
        nearestAhead = other;
...
function updateExitPenalty(state, turn, dt) {
    if (!turn) {
        state.exitPenalty = Math.max(0, state.exitPenalty - dt * 0.32);
        return;
    }
    if (turn.distance > 0 && turn.distance < 58) {
        const speedOvershoot = clamp01((state.speed - state.targetSpeed) / 9);
        const lineError = clamp01(Math.abs(state.laneTarget - state.lane) / 1.8);
        const skillPenalty = clamp01((1.02 - state.driver.cornerSkill) * 1.8);
        const demand = clamp(speedOvershoot * 0.5 +
            lineError * 0.22 +
            skillPenalty * 0.12, 0, 0.52);
        state.exitPenalty += clamp(demand - state.exitPenalty, -0.24 * dt, 0.9 * dt);
        return;
    }
    const recovering = turn.distance <= 0 && turn.distance > -62;
    state.exitPenalty = Math.max(0, state.exitPenalty -
        dt *
            (recovering
                ? 0.08 + state.driver.traction * 0.12
                : 0.3));
}
function desiredAcceleration(state, turn, deltaSpeed) {
    if (deltaSpeed < -0.25) {
    ...
    const cornerZone = turn && turn.distance > -54 && turn.distance < 48;
    const exitZone = turn && turn.distance <= 6 && turn.distance > -62;
    if (exitZone && turn) {
        const exitProgress = clamp01((-turn.distance + 6) / 68);
        const progressiveBase = 2.15 + exitProgress * 3.0;
        const tractionScale = state.driver.traction *
            state.driver.exitAttack *
            (1 - state.exitPenalty * 0.72);
        return Math.min(progressiveBase *
            state.driver.launch *
            tractionScale, deltaSpeed * 1.05);
    }
    const base = cornerZone ? 2.9 : 5.1;
    return Math.min(base * state.driver.launch, deltaSpeed * 1.15);
}
function updateLongitudinal(state, turn, dt) {
    const deltaSpeed = state.targetSpeed - state.speed;
    const desired = desiredAcceleration(state, turn, deltaSpeed);
    const jerk = desired < state.acceleration ? 18 : 6.5;
    state.acceleration += clamp(desired - state.acceleration, -jerk * dt, jerk * dt);
    ...
    if (state.speed > state.targetSpeed + 0.8) {
        state.speed = Math.max(state.targetSpeed, state.speed - 7.5 * dt);
    }
}
export function createCompetition({ CL, TURNS, opponents, startLineS, gridOffset, }) {
    const states = opponents.map((visual, index) => {
    ...
    function presentState(state) {
        ...
        const aheadLane = state.lane + clamp(state.laneVelocity * 0.75, -0.7, 0.7);
        ...
        state.steer = clamp(wrapAngle(aheadYaw - centreYaw) * 1.55 +
            state.laneVelocity * 0.08, -0.42, 0.42);
        state.visual.carRoot.position.set(x, 0, z);
        state.visual.carRoot.rotation.y = -yaw;
        state.visual.carBody.rotation.x = 0;
        state.visual.carBody.rotation.z = clamp(state.steer * 0.032, -0.032, 0.032);
    ...
    function step(dt) {
        for (const state of states) {
            ...
            state.laneTarget = clamp(state.driver.lineBias +
                state.baseLane * 0.08 +
                raceLine +
                interaction.laneBias, -2.25, 2.25);
            const laneError = state.laneTarget - state.lane;
            const stiffness = interaction.laneBias === 0 ? 3.0 : 4.4;
            const damping = interaction.laneBias === 0 ? 3.7 : 4.2;
            state.laneVelocity +=
                (laneError * stiffness -
                    state.laneVelocity * damping) *
                    dt;
            state.laneVelocity = clamp(state.laneVelocity, -1.25, 1.25);
            state.lane += state.laneVelocity * dt;
            state.targetSpeed = Math.min(targetSpeedFor(state, CL, TURNS), interaction.speedCap);
            ...
            const nextS = wrapS(state.s + state.speed * dt, CL.length);
            const nextProgress = wrapS(nextS - startLineS, CL.length);
            const crossedLine = state.progress > CL.length * 0.82 &&
    ...
    function getClassification(playerLap, playerProgress) {
        const entries = [
            {
                id: 'player',
                label: 'YOU',
...
//# sourceMappingURL=createCompetition.js.map