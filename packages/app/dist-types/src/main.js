...
import { CFG, clamp, clamp01, NODES, buildCentreline, makeLocator, createVehicle, stepVehicle, applyBarriers, gearFor, fmtTime } from '@indygp/core';
...
try {
    ...
    fatal('The circuit could not be built', 'Construction failed before the session started.<br><br><code>' +
...
function boot() {
    ...
    const initialSpeedUnit = storedUnit === 'mph' ? 'mph' : 'kph';
    ...
    DOM.mapsheet.addEventListener('click', () => { if (SESSION.mapOpen)
        toggleMap(); });
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.hidden && SESSION.running && !SESSION.paused)
            togglePause();
    });
        ...
        if (tp) {
            ...
            if (Number.isFinite(tx) && Number.isFinite(tz)) {
                ...
                setTimeout(() => {
                    ...
                    if (flip)
                        car.yaw += Math.PI; // face the opposite way
                }, 400);
            }
        }
    }
    ...
    function updateLap(prog, dtMs) {
        ...
        if (crossedForward && car.vLong > 0) {
            if (SESSION.lap > 0 && SESSION.armed) {
                ...
                if (SESSION.best == null || SESSION.clock < SESSION.best)
                    SESSION.best = SESSION.clock;
    ...
    function updateCamera(dt) {
        ...
        if (!camState.ready) {
            camState.pos.copy(tmpV);
            camState.look.copy(tmpLook);
            camState.ready = true;
        }
    ...
    function frame(now) {
        ...
        prevT = now;
        if (dt > CFG.sim.maxFrame)
            dt = CFG.sim.maxFrame; // never let a stall teleport the car
        ...
        if (active) {
            ...
            while (acc >= CFG.sim.step && guard++ < 24) {
                ...
                if (hit > 0)
                    SESSION.shake = Math.max(SESSION.shake, hit);
            ...
            Audio.update(clamp01(startRamp * (0.18 + bandRev * 0.82)), INPUT.throttle * 0.8 + (car.slipping ? 0.2 : 0));
        }
        // present
        ...
        for (const p of frontAxle)
            p.rotation.y = -car.steer;
        for (const w of allWheels)
            w.rotation.z = -car.wheelSpin;
        competition.present();
